...
async function createGame() {
    try {
        ...
        for (let i = 0; i < category.length; i++) {
            let res = await axios.get(`https://jservice.io/api/category?id=${category[i][1]}`);
            onBoard.push(res);
            let $colum = $(`
...
function populateCard() {
    ...
    for (let j = 0; j < category.length; j++) {
        ...
        for (let i = 0; i < 5; i++) {
            let index = onBoard[i].data.clues[Math.floor(Math.random() * onBoard[i].data.clues.length)];
            if (index.value) {
                value = index.value;
            }
            let $item = $(`<div id="${index.id}" class="card"> <h3>${value}</h3> </div>`);
            $columHolder.append($item);
            chosenOne.push({ id: index.id, question: index.question, answer: index.answer, value: value });
        }
    }
}